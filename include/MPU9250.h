...
// mpu9250 registers
#define MPUREG_SELF_TEST_XG         0x00
#define MPUREG_SELF_TEST_YG         0x01
#define MPUREG_SELF_TEST_ZG         0x02
#define MPUREG_SELF_TEST_XA         0x0D
#define MPUREG_SELF_TEST_YA         0x0E
#define MPUREG_SELF_TEST_ZA         0x0F
#define MPUREG_XG_OFFS_H            0x13
#define MPUREG_XG_OFFS_L            0x14
#define MPUREG_YG_OFFS_H            0x15
#define MPUREG_YG_OFFS_L            0x16
#define MPUREG_ZG_OFFS_H            0x17
#define MPUREG_ZG_OFFS_L            0x18
...
public:
    ...
    MPU9250(SPIClass *spi_, long clock, uint8_t cs, uint8_t low_pass_filter = BITS_DLPF_CFG_188HZ, uint8_t low_pass_filter_acc = BITS_DLPF_CFG_188HZ){
        spi = spi_;
        my_clock = clock;
        my_cs = cs;
    ...
    bool WriteReg(uint8_t WriteAddr, uint8_t WriteData );
    uint8_t ReadReg(uint8_t ReadAddr);
    void ReadRegs(uint8_t ReadAddr, uint8_t *ReadBuf, unsigned int Bytes );
 
    bool init(bool calib_gyro = true, bool calib_acc = true);
    int auto_init(uint8_t AccelScale = BITS_FS_8G, uint8_t GyroScale = BITS_FS_500DPS);
    void read_temp();
    void read_acc();
    void read_gyro();
    uint16_t set_gyro_scale(uint8_t scale);
    uint16_t set_acc_scale(uint8_t scale);
    void calib_acc();
    void select();
    void deselect();
    unsigned int whoami();
    ...
    bool accel_calibrate(int sample);
    bool accelangle_calibrate(int sample);
    bool gyro_calibrate(int sample);
    void MeaurementAccelBias(int sample);
    ...
    float a_bias[3];
    float a_scale[3];
    float accel_lowdata[3];
    float g_bias[3];
    float gyro_lowdata[3];
    ANGLE a_angle_bias;
...
public: // Kalman Filter
    ANGLE accelangle;
    ANGLE gyroangle;
    ANGLE KalmanAngle;
    ANGLE KalmanUncertaintyAngle;
...
private:
    ...
    void CarculateAngle();
    void CarculateAngleGyro(float deltatime);
